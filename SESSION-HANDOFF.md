...
- **Next exact step — all three design Moves are COMPLETE & APPROVED.** Phase F approved 2026-06-19;
  ...
  line has no open items. **Two post-Move Settings features are implemented + committed locally → In Review:**
  **`ONE-63`** (project deletion — Danger Zone, type-to-confirm; cascade delete verified on the live DB → no
  orphans; success toast + redirect) and **`ONE-64`** (project rename — a "General" card above the Danger
  Zone; `renameProject` updates `Project.name` only, owner-scoped, 1–60 chars, success toast, no refresh).
  Both reuse the existing design system + `radix-ui` — **no new dependency** (the ONE-63 `Dialog` is shared
  infra). The broader product backlog (marketing / onboarding / Paddle go-live) is separate, pre-existing
